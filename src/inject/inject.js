var app = {
  ...
  injectForProfile: function() {
    ...
    buttonEl.querySelector('button').addEventListener('click', function(e) {
      e.stopPropagation();
      e.preventDefault();

      var profile = self.scrapeProfile();
  ...
  scrapeConnections: function() {
    ...
    var number = this.textContent(connectionEl.querySelector('.shared'));
  ...
  formatExperience: function(experience) {
    ...
    for(var i = 0, l = experience.length; i < l; ++i) {
      out += experience[i].title + ' - ' + experience[i].company + ' - ' + experience[i].time + '\n';
    }
